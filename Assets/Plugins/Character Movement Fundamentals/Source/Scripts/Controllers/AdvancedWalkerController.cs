...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller
	{
		...
		protected bool jumpKeyWasLetGo = false;
		protected bool jumpKeyIsPressed = false;
		...
		protected Vector3 savedMovementVelocity = Vector3.zero;
		...
		public ControllerState CurrentControllerState 
		{
			...
			protected set
			{
				_currentControllerState = value;
				OnStateChanged?.Invoke(_currentControllerState);
			}
		}

		public Action<ControllerState> OnStateChanged;

		public ControllerState _currentControllerState = ControllerState.Falling;
		...
		protected virtual void HandleJumpKeyInput()
		{
		...
		protected virtual void OnGroundContactRegained(Vector3 _collisionVelocity)
		{
			//Call 'OnLand' event;
			if (OnLand != null)
		...
		public virtual Vector3 GetMomentum()
		{
			Vector3 _worldMomentum = momentum;
			if (useLocalMomentum)
				_worldMomentum = tr.localToWorldMatrix * momentum;
		...
		public virtual void AddMomentum(Vector3 _momentum)
		{
			if (useLocalMomentum)
				momentum = tr.localToWorldMatrix * momentum;
